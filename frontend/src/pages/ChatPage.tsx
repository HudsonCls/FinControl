    ...
    <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[78%] rounded-2xl px-3.5 py-2 text-sm leading-relaxed ${
          mine
            ? 'rounded-br-sm bg-brand-100 text-emerald-900'
...
export default function ChatPage() {
  ...
  return (
    <Layout title="Chat WhatsApp">
      <div className="mx-auto flex h-full max-w-2xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-4 py-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand text-white">
            <MessageCircle size={18} />
          </div>
          <div>
            <div className="text-sm font-semibold text-slate-800">FinControl IA</div>
        ...
        <div
          ref={scrollRef}
          className="flex-1 space-y-2 overflow-y-auto bg-brand-50 p-4"
        >
        ...
        <div className="border-t border-slate-200 bg-white p-3">
          <div className="mb-2 flex flex-wrap gap-1.5">
            {SUGGESTIONS.map((s) => (
              <button
                key={s}
                onClick={() => quick(s)}
                className="flex items-center gap-1 rounded-full border border-slate-200 px-2.5 py-1 text-xs text-slate-600 hover:border-brand hover:bg-brand-50"
              >
          ...
          <form onSubmit={submit} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-brand focus:bg-white"
            />
            <button
              type="submit"
